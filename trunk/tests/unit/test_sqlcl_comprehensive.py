"""
Comprehensive tests for lsc.sqlcl — SQL client utilities for SDSS queries.
...
# ---------------------------------------------------------------------------
# Helper to inject sys into sqlcl module (it uses sys without module-level import)
# ---------------------------------------------------------------------------
...
class TestFiltercommentEdgeCases:
    ...
    def test_very_long_query(self):
        """Handle a very long SQL query without issues."""
        from lsc.sqlcl import filtercomment
...
class TestQueryFunction:
    ...
    def test_comments_stripped_before_query(self):
        ...
        with patch('urllib.request.urlopen', return_value=fake_response) as mock_open:
            ...
            call_url = mock_open.call_args[0][0]
            # Comment text should not appear in URL
    ...
    @pytest.mark.http
    def test_very_long_query(self):
        """Very long queries should still be sent without error."""
        from lsc.sqlcl import query
    ...
    def test_special_characters_in_query(self):
        """Special chars like &, =, ? should be properly encoded."""
        from lsc.sqlcl import query
        fake_response = io.BytesIO(b"col\nval\n")
        with patch('urllib.request.urlopen', return_value=fake_response) as mock_open:
            query("SELECT * FROM t WHERE name = 'a&b'")
            call_url = mock_open.call_args[0][0]
            # The & should be encoded, not treated as URL parameter separator
            assert mock_open.called
...
class TestMainBasic:
    ...
    def test_skip_first_line(self, capsys):
        ...
        # Data line should still be present
...
class TestMainOutput:
    ...
    def test_no_query_no_output(self, capsys):
        ...
        main(['sqlcl'])
        out = capsys.readouterr().out
        # No queries means no output (unless verbose)
