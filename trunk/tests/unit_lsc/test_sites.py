"""
Tests for lsc.sites — extinction tables, filter mappings,
...
class TestExtinctionDefaultSite:
    ...
    def test_extinction_default_not_empty(self):
...
class TestChosecolorBasic:
    ...
    def test_ri_not_possible_without_I(self):
        from lsc.sites import chosecolor
        color = chosecolor(["B", "V", "R"])
        assert "RI" not in color.get("R", [])

    def test_missing_pair_single_filter(self):
...
class TestChosecolorUsegood:
    ...
    def test_usegood_fallback_when_preferred_unavailable_v_alone(self):
        """With usegood=True and only one filter available, result should be empty."""
        from lsc.sites import chosecolor
        color = chosecolor(["V"], usegood=True)
        assert color["V"] == []

    def test_usegood_multiple_pairs_collapses(self):
        """With usegood=True and multiple possible pairs, each filter collapses to one."""
        from lsc.sites import chosecolor
...
# ---------------------------------------------------------------------------
# Extra chosecolor and filter mapping tests
# ---------------------------------------------------------------------------

class TestFilterMappingsExtra:
    ...
    def test_filterst_sloan_completeness(self):
        """Sloan system should include instrument filter names for ugrizw."""
        from lsc.sites import filterst
        for name in ['up', 'SDSS-U', 'gp', 'SDSS-G', 'rp', 'SDSS-R', 'ip', 'SDSS-I', 'zs', 'w']:
            assert name in filterst['sloan']

    def test_filterst_apass_completeness(self):
        """APASS system should include instrument filter names for BVgriw."""
        from lsc.sites import filterst
        for name in ['B', 'Bessell-B', 'V', 'Bessell-V', 'gp', 'SDSS-G', 'rp', 'SDSS-R', 'ip', 'SDSS-I', 'w']:
